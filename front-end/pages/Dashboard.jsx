...
function EventBlock({
  ...
  // }
  // attendees = newGuests;
  let guest_string = attendees
    ? attendees.map((attendees) => attendees.email).join(", ")
    : "";
...
function ExpandedEvent({
  ...
  function handleClick() {
    ...
    const new_attendees = rsvp_status
      ? attendees.filter((a) => a.id !== tokenInfo.userId)
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (tokenInfo.token && tokenInfo.userId && tokenInfo.userEmail) {
      fetch(
        `http://localhost:4000/user/${tokenInfo.userId}/groups_events?full=True`,
        {
          method: "GET",
          headers: {
          ...
          if (data) {
            ...
            for (const g of data) {
              for (const e of g.events) {
                initialRSVPs[e.event_id] = e.attendees.some(
                  (a) => a.id === tokenInfo.userId
                );
              }
            }
  ...
  async function RSVP() {
    ...
    const newAttendeesList = rsvp_status
      ? popup.guests.filter((a) => a.id !== tokenInfo.userId).map((a) => a.id)
      : [...popup.guests.map((a) => a.id), tokenInfo.userId];
    console.log(
      JSON.stringify({
    ...
    for (const e of filteredEvents) {
      if (e.event_id === popup.id) {
        ...
        const new_attendees = rsvp_status
          ? e.attendees.filter((a) => a.id !== tokenInfo.userId)
          : [...e.attendees, currentAttendee];
        ...
        <div className="eventArea">
          ...
          {eventItems.length > 0 ? (
            ...
            <div className="no-events-message">
              ...
              <span> No events to show {"\u{1F61E}"}</span>
            </div>
          )}
        </div>
      </div>
